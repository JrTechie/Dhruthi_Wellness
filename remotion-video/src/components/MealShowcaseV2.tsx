...
export const MealShowcaseV2: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  const switchProgress = interpolate(frame, [55, 75], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  const activeImage = switchProgress < 0.5 ? "lunch.jpeg" : "evening.jpeg";
  const activeStepNum = switchProgress < 0.5 ? "MEAL 04 OF 05" : "MEAL 05 OF 05";
  const activeHeading = switchProgress < 0.5 ? "LUNCH" : "EVENING SNACK";

  const microY = interpolate(frame, [0, 120], [0, -3]);

  const entranceSpring = spring({
    frame: frame < 60 ? frame - 4 : frame - 60 - 4,
    fps,
