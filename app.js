...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- 14. Smooth Scroll Reveal & Staggered Entrance Engine ---
  function initScrollReveal() {
    ...
    // Auto-assign staggered delays to grid items
    const gridContainers = document.querySelectorAll('.services-grid, .about-credentials, .pricing-grid, .testimonials-grid, .testimonials-glass-grid, .blog-grid, .nutrition-grid-1to1, .hero-stats');
    ...
    // Attach reveal-on-scroll class to target elements
    ...
    // Create IntersectionObserver for smooth scroll reveals
    ...
    document.querySelectorAll('.reveal-on-scroll, .reveal-scale, .reveal-slide-left, .reveal-slide-right, .wipe-baseline-wrapper, .wipe-baseline-img, .text-reveal-up').forEach(el => {
      revealObserver.observe(el);
    });

    // Auto-trigger entrance for above-the-fold hero elements
    setTimeout(() => {
      document.querySelectorAll('.wipe-baseline-wrapper, .wipe-baseline-img, .text-reveal-up').forEach(el => {
  ...
  // --- 15. Scroll Reading Progress Bar Handler ---
  ...
  // --- 16. Program Category Filter Handler ---
  ...
  // --- 17. Interactive Typewriter Tagline Animation (Inspired by Jillian Greaves) ---
  ...
  // --- 18. Interactive Bento Tile Mouse Spotlight Tracking ---
  document.querySelectorAll('.interactive-tile, .artifact-card, .service-card, .cred-card, .trust-card, .philosophy-card, .blog-card').forEach(tile => {
