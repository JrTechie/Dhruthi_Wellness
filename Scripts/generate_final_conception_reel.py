...
TEMP_VIDEO = os.path.join(ARTIFACTS_DIR, "temp_conception_clean_raw.mp4")
...
async def generate_all_voiceovers():
    print("Step 1: Synthesizing voiceover audio segments...")
    audio_clips = {}
    audio_envelopes = {}
...
def build_final_video(audio_clips, audio_envelopes, avatar_obj):
    print("Step 2: Rendering frames & writing video file...")
    FPS = 25
    ...
    for scene_id in range(1, 6):
        ...
        print(f"Scene {scene_id} frames compiled.")
        
    video_writer.release()
    print("Raw video writing completed.")
    
    print("Step 3: Merging audio voiceover segments...")
    ...
    print(f"Exporting final video to {FINAL_VIDEO}...")
    ...
    print(f"\n🎉 Instagram Reel successfully generated!")
    print(f"File 1 (Workspace): {FINAL_VIDEO}")
    print(f"File 2 (Artifacts): {ARTIFACT_VIDEO}")

async def main():
    avatar_obj = FastLiveAvatar(AVATAR_PATH)
    audio_clips, audio_envelopes = await generate_all_voiceovers()
    build_final_video(audio_clips, audio_envelopes, avatar_obj)
